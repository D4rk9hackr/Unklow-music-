...
object YTPlayerUtils {
     ...
     * IOS client is reliable, does not trigger Botguard verification, and
     * provides direct stream URLs without signature cipher complexity.
     */
    ...
    /**
     * Fallback client order: IOS and WEB_REMIX are prioritised as primary streaming
     * targets. ANDROID_VR, TVHTML5, and TVHTML5_SIMPLY_EMBEDDED_PLAYER are removed
     * because they consistently return LOGIN_REQUIRED / UNPLAYABLE errors.
     */
    private val STREAM_FALLBACK_CLIENTS: Array<YouTubeClient> = arrayOf(
        WEB_REMIX,           // Primary web fallback — reliable with PoToken
        IOS,                 // Primary mobile fallback — direct URLs, no PoToken
        WEB_CREATOR,         // Authenticated web client — works for age-restricted
        ANDROID_CREATOR,     // Android creator — children's / paid content
        IPADOS,              // iPad variant of IOS
        MOBILE,              // Standard Android client
        WEB                  // Bare web client — last resort
    )
    ...
    private suspend fun resolvePlaybackData(
        ...
        // Generate PoToken ONLY if MAIN_CLIENT uses it (IOS does not require PoToken)
        ...
        // Try MAIN_CLIENT (IOS) for fast stream resolution and METADATA_CLIENT (WEB_REMIX) for
        ...
        // NOTE: Do NOT include LOGIN_REQUIRED here — some clients return LOGIN_REQUIRED as a
        ...
        // For private tracks: start with WEB_REMIX (index 0) which handles private content via PoToken
        // For age-restricted: start with WEB_REMIX (index 0) which can bypass with authentication
        // For normal content: start with MAIN_CLIENT (IOS) at index -1
        val startIndex = when {
            isPrivateTrack -> 0  // WEB_REMIX
            isAgeRestricted -> 0
            else -> -1
        }
    ...
    suspend fun playerResponseForMetadata(
        ...
        return YouTube.player(videoId, playlistId, WEB_REMIX, signatureTimestamp.timestamp, poToken?.playerRequestPoToken) // IOS does not work with history
